...
public class ManipuladorImagensApp extends Application {
	...
	private void criarComponentes(Group noRoot) {
		...
		btPlay.setLayoutX(100);
		btPause.setLayoutX(200);
		btStop.setLayoutX(300);
		
		btPlay.setLayoutY(450);
		btPause.setLayoutY(450);
		btStop.setLayoutY(450);
		//Criando as ações dos botões
		btPlay.setOnAction((ev)->{ mp.play(); });
		btPause.setOnAction((ev)->{ mp.pause(); });
		btStop.setOnAction((ev)->{ mp.stop(); });
		//Adicionando no nó root
		noRoot.getChildren().addAll(mv,btPlay,btPause,btStop);
	}
